...
const AddFoodScreen = ({navigation, route}: any) => {
  ...
  const localFoodItems = useSelector<LocalStore, FoodItem[]>((state: LocalStore) =>
    getFoodSelector(state, loadBatch, searchText)
  )

  const [foodItems, setFoodItems] = useState(localFoodItems)
  const [isLoading, setIsLoading] = useState(false)

  const searchFoodsDebounce = useCallback(
    debounce(
      (text: string, localItems: FoodItem[], batch: number, runSearch: boolean) => {
        if (!runSearch) {
          return
        }
  ...
  const renderFooter = () => <View style={{marginBottom: Spacing.X_LARGE}} />
